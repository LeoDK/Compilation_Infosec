...
(* [cfg_expr_of_eexpr e] converts an [Elang.expr] into a [Cfg.expr res]. This should
...
let rec cfg_node_of_einstr (next: int) (cfg : (int, cfg_node) Hashtbl.t)
  ...
  | Elang.Ideclaration v ->
    Hashtbl.replace cfg next (Cassign(v, Eint 0, succ));
    OK (next, next+1)
...
let cfg_fun_of_efun { funargs; funbody; funvartype; funrettype } =
